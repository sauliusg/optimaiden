...
DATABLOCK *new_datablock( const char *name, DATABLOCK *next, cexception_t *ex )
{
    ...
    cexception_guard( inner ) {
        ...
        datablock->last_datablock_in_stream = false;
        datablock->next = next;
    }
    cexception_catch {
...
/*DATABLOCK *datablock_disconnect( DATABLOCK *datablock )
{
    if ( datablock ) {

        printf("[C] Disconnecting block: %s\n", datablock_name( datablock ));
        datablock_set_next( datablock, NULL );
    }
    printf( "[C] Finished disconnecting block: %s\n", datablock_name( datablock ) );

    if( datablock->next == NULL ) {
      printf("[C} Successfully disconected block: %s\n", datablock_name( datablock ));
    }
...
ssize_t datablock_get_tag_value_length( DATABLOCK *datablock, int tag_nr )
{
    assert( datablock );
    assert( tag_nr >= 0 && tag_nr < (int)datablock->length );
    return datablock -> value_lengths[tag_nr];
}
...
char *datablock_cifvalue_get_string( DATABLOCK *datablock, int tag_nr, int value_nr )
{
    assert( datablock );
    assert( tag_nr >= 0 && tag_nr < (int)datablock->length );
    assert( value_nr >= 0 && value_nr < datablock->value_lengths[tag_nr] );

    return value_get( datablock_cifvalue( datablock, tag_nr, value_nr ));
}
...
char *datablock_get_tag_name( DATABLOCK *datablock, int tag_nr ) {
    assert( datablock );
    assert( tag_nr >= 0 && tag_nr < (int)datablock->length );
    return datablock->tags[tag_nr];
}
...
void ada_queue_insert_datablock( DATABLOCK *datablock )
{
    assert( datablock );
    enqueue_datablock( datablock );
}

int datablock_get_if_last_in_stream( DATABLOCK *datablock )
{
    assert( datablock );

    if (datablock->last_datablock_in_stream) {
      return 0;
    } else {
      return 1;
    }
...
DATABLOCK *datablock_set_last_in_stream ( DATABLOCK *datablock )
{
    assert( datablock );
    datablock->last_datablock_in_stream = true;
    return datablock;
}
