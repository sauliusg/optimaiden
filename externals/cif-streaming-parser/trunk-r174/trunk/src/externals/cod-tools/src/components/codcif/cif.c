...
DATABLOCK *cif_get_current_datablock ( CIF *cif )
{
  return cif->current_datablock;
}

DATABLOCK *obtain_current_datablock_from_cif( CIF *cif )
{
    assert( cif );
    assert( cif->current_datablock );
    assert( !datablock_next( cif->current_datablock ) );
    DATABLOCK *return_datablock = cif->current_datablock;
    cif->current_datablock = NULL; 
       
    assert( cif->last_datablock == return_datablock );
    cif->last_datablock = NULL;
    
    assert( cif->datablock_list == return_datablock );
    cif->datablock_list = NULL; 

    return return_datablock;
}
